...
long AmsRouter::AddSyntheticNotification(uint16_t port, const AmsAddr& addr, uint32_t* pNotification, std::shared_ptr<SyntheticNotification> notify)
{
...
long AmsRouter::DelSyntheticNotification(uint16_t port, const AmsAddr* pAddr, uint32_t hNotification)
{
    auto& p = ports[port - Router::PORT_BASE];
