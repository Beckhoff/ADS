                                            ...
                                            uint32_t *pNotification)
{
    ...
    try {
        auto notify = std::make_shared<SyntheticNotification>(pFunc, hUser, *pAddr, (uint16_t)port, nType);
...
{
    ASSERT_PORT_AND_AMSADDR(port, pAddr);	
    return GetRouter().DelSyntheticNotification((uint16_t)port, pAddr, hNotification);
}
